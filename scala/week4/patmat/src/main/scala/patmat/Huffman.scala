...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    chars.distinct zip chars.distinct.map(head => chars.count(head => head.equals()))
  }
  /**
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case left :: right :: remain => (makeCodeTree(left, right) :: remain).sortWith((t1, t2) => weight(t1) < weight(t2))
    case _ => trees
  }
